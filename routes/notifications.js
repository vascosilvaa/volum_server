...
var returnRouter = function (io) {
    ...
    app.get('/', passport.authenticate('jwt'), function (req, res) {
        ...
        db.get().query({ sql: 'SELECT * FROM notifications INNER JOIN users ON notifications.id_user2 = users.id_user INNER JOIN vols ON notifications.id_vol = vols.id_vol WHERE notifications.id_user = ? AND notifications.type = 1', nestTables: true }, [req.user.id_user], function (error, results, fields) {
            console.log(error);
